...
const OrdersDialog = (props) => {
    ...
    const getPatientOrders = useCallback(() => {
        const fetchData = async (patientId, page) => {
            return await OrderService.getAll(patientId, {
                page,
                limit,
                sort: 'desc',
            });
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            if (!rowCount || currentPage !== page || !isLatest) {
                const fetchResult = await getPatientOrders(patientId, page);
                setRowCount(fetchResult.count);
                setRows(fetchResult.data);
